/**
 * "이 코드"에서 node의 3가지 (Main node, empty node, node)들 이 서로 밀어내는 효과가 적용 될때 우선순위를 정할 수 있는지.
우선순위를 정할 수 있다면 1. Main node 2. node 3. empty node순서로 우선위를 정해줘.
 */
...
const MainGraph: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      // 1) forceManyBody에서 '힘의 세기'를 노드 타입별로 다르게 설정
      .force(
        "charge",
      ...
      // 2) forceCollide로 '충돌 반경'도 노드 타입별로 다르게 지정
      .force(
        "collide",
        d3
          .forceCollide()
